...
Die Passwörter werden nicht im öffentlichen Quellcode gespeichert. GitHub hält sie als verschlüsselte Environment-Secrets und überträgt sie ausschließlich bei einem manuell gestarteten, autorisierten Workflow an das Dashboard.

## Einmalige Einrichtung

1. Im Repository `roruffm/FCG-Dash` **Settings** öffnen.
2. Links **Environments** wählen und ein Environment mit dem Namen `production` anlegen.
3. Unter **Environment secrets** diese drei Secrets anlegen:
   - `FCG_LEADERSHIP_PASSWORD`
   - `FCG_STAFF_PASSWORD`
   - `FCG_GENERAL_PASSWORD`
4. Für jedes Secret ein eigenes Passwort mit 12 bis 256 Zeichen eintragen.
5. Unter **Deployment protection rules** nach Möglichkeit einen oder mehrere erforderliche Prüfer festlegen. So kann eine Passwortänderung erst nach Freigabe ausgeführt werden.

## Passwort später ändern

1. **Settings → Environments → production** öffnen.
...
- Nur die GitHub-Secrets unter dem Environment `production` verwenden.
